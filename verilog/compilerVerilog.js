...
/**
 * 编译并保存 Verilog 文件到 compiled_code 文件夹
 */
async function compileAndDownloadVerilog() {
    try {
        const verilogCode = compileToVerilog();
        const result = await downloadTextAsFile(verilogCode, 'blueprint_design.v');
        
        if (result && result.success) {
            alert(`Verilog 代码已生成并保存到: ${result.path}`);
            console.log('Verilog 代码已生成并保存');
        } else {
            alert('Verilog 代码已生成，但保存文件时遇到问题。请检查控制台获取详细信息。');
            console.log('Verilog 代码已生成');
        }
    } catch (error) {
